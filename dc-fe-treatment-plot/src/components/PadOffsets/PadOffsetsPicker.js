...
const PadOffsetsPicker = ({ onSave, currentOffsetSetting, assets, companyId }) => {
  ...
  const handleSave = () => {
    ...
    let newOffsetAssets = currentOffsetSetting.filter(
      setting => !newOffsets.find(newOffset => newOffset.selectedWellId === setting.selectedWellId)
    );
    newOffsetAssets = [...newOffsetAssets, ...newOffsets];

    closeDialog();
    onSave(newOffsetAssets);
  };
            ...
            <FormControl margin="normal" disabled={isEmpty(selectedPadIds)} fullWidth required>
              ...
              >
                {requestContext.wellsData.map(well => {
                  const isMenuItemDisabled = currentOffsetIds.includes(well.asset_id);
                  return (
