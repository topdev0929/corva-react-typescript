import { getUserAutocomplete } from '~/clients/jsonApi';
...
export const handleFetchUsers = async (search, callback, companyId) => {
  ...
  let res;
  try {
    res = await getUserAutocomplete(null, search, companyId);
  } catch (e) {
    console.error(e);
  }
...
export const isSuggestionsListOpened = () => {
  const elements = document.getElementsByClassName(styles.mention__suggestions__list);
