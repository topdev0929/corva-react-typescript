...
function MultiSelect({ options, currentValues, title, size, renderValue, onChange }) {
    ...
    <FormControl className={classes.formControl}>
      ...
      >
        <MenuItem key="all" value="all">
          <Checkbox checked indeterminate={values.length > 0} className={classes.checkBox} />
          <ListItemText
        ...
        {options.map(option => (
          <MenuItem key={option} value={option}>
            <Checkbox checked={values.indexOf(option) > -1} className={classes.checkBox} />
